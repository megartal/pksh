...
class ControllerSaleOrder extends Controller
{
    ...
    public function info()
    {
        ...
        if ($order_info) {
            ...
            //changed!
            $products = $this->model_sale_order->getOrderProductsOfVendor($this->request->get['order_id'], $this->user->getId());

			$newTotal = 0;
            foreach ($products as $product) {
                // changed! if is added
                // if ($this->user->getId() === $product['vendor_id']) {
                    $option_data = array();
            ...
            // changed! for commented
            // foreach ($totals as $total) {
                $data['totals'][] = array(
                    'title' => $this->language->get('column_total'),
                    'text'  => $this->currency->format($newTotal, $order_info['currency_code'], $order_info['currency_value']),
                );
            // }
            $data['total'] = $newTotal;
    ...
    protected function validate()
    {
        ...
        if (isset($this->request->get['api'])) {
            if (substr($this->request->get['api'], 0, 17) == 'api/order/history') {
                return true;
            }
        }
        //End add
    ...
    public function history()
    {
        ...
        //changed!
        $results = $this->model_sale_order->getOrderHistories($this->request->get['order_id'], ($page - 1) * 10, 10, $this->user->getId());
    ...
    public function changeOrderStatus()
    {
        ...
        $data['change_status'] = $this->request->post['order_status_id'];
        //changed!
        // $data['comment'] = $this->request->post['comment'];
        $names = $this->model_sale_order->getVendorName($this->user->getId());
        $data['comment'] = $names->row['name'];
        $data['notify']  = $this->request->post['notify'];
        //changed!
        $this->model_sale_order->addOrderHistory($order_id, $data, $this->user->getId());
