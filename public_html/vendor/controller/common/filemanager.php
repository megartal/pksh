...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/'.$this->user->getId().'/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . 'catalog/'.$this->user->getId();
		}
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'.$this->user->getId().'/'))) . $url, 'SSL')
				);
	...
	public function delete() {
		...
		foreach ($paths as $path) {
			...
			// Check path exsists
			if ($path == DIR_IMAGE . 'catalog/'.$this->user->getId()) {
