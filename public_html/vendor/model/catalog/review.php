...
class ModelCatalogReview extends Model {
	//Start add
	public function getUserReviews($review_id) {
		$query = $this->db->query("SELECT p.vendor_id FROM " . DB_PREFIX ."review r LEFT JOIN " . DB_PREFIX ."product p ON (r.product_id = p.product_id) WHERE r.review_id = '" . (int)$review_id . "' AND p.vendor_id = '" . (int)$this->user->getId() . "'");
        if($query->num_rows){
			return true;
		} else {
			return false;
	...
	public function getReviews($data = array()) {
        ...
        //Start add
		$sql.= " AND EXISTS (SELECT p.vendor_id FROM " . DB_PREFIX . "product p  WHERE p.vendor_id = '". (int)$this->user->getId() ."' AND p.product_id = r.product_id)";
        //End add
