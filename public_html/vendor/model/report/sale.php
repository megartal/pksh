...
class ModelReportSale extends Model {
	...
	public function getTotalSales($data = array()) {
		...
		$sql .= " AND EXISTS (SELECT op.order_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND op.order_id=o.order_id)";
	...
	public function getTotalOrdersByCountry() {
		$query = $this->db->query("SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) WHERE EXISTS (SELECT op.order_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND op.order_id=o.order_id) AND o.order_status_id > '0' GROUP BY o.payment_country_id");
	...
	public function getTotalOrdersByDay() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` o WHERE EXISTS (SELECT op.order_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND op.order_id=o.order_id) AND  order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) = DATE(NOW()) GROUP BY HOUR(date_added) ORDER BY date_added ASC");
	...
	public function getTotalOrdersByWeek() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` o WHERE EXISTS (SELECT op.order_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') GROUP BY DAYNAME(date_added)");
	...
	public function getTotalOrdersByMonth() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` o WHERE EXISTS (SELECT op.order_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND op.order_id=o.order_id) AND  order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(date_added)");
	...
	public function getTotalOrdersByYear() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` o WHERE EXISTS (SELECT op.order_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND order_status_id IN(" . implode(",", $implode) . ") AND YEAR(date_added) = YEAR(NOW()) GROUP BY MONTH(date_added)");
...
//End add
	...
	public function getTotalOrders($data = array()) {
		...
		switch($group) {
			case 'day';
				$sql = "SELECT COUNT(DISTINCT YEAR(date_added), MONTH(date_added), DAY(date_added)) AS total FROM `" . DB_PREFIX . "order` o";
				break;
			default:
			case 'week':
				$sql = "SELECT COUNT(DISTINCT YEAR(date_added), WEEK(date_added)) AS total FROM `" . DB_PREFIX . "order` o";
				break;
			case 'month':
				$sql = "SELECT COUNT(DISTINCT YEAR(date_added), MONTH(date_added)) AS total FROM `" . DB_PREFIX . "order` o";
				break;
			case 'year':
