...
class ModelSaleOrderProduct extends Model {
	...
	public function getOrder($order_product_id) {
		$query = $this->db->query("SELECT o.* FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id=o.order_id) WHERE order_product_id = '".(int)$order_product_id."' LIMIT 1");
	    return $query->row;
	}  
	...
	public function addOrderHistory($order_product_id, $data) {
		$check = $this->db->query("SELECT op.vendor_id, op.product_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND op.order_product_id ='".(int)$order_product_id."'");
		if(!$check->num_rows){
			die("Unsuported access");
		}
		$product_id = $check->row['product_id'];
