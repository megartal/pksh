...
class ModelSaleReturn extends Model {
	//Start add
	public function getUserReturns($return_id) {
	    $query = $this->db->query("SELECT op.order_id FROM `" . DB_PREFIX . "return` r LEFT JOIN " . DB_PREFIX . "order_product op  ON (op.order_id=r.order_id) WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND op.model=r.model AND r.return_id='". (int)$return_id ."'");
	    if($query->num_rows){
			return true;
		} else {
			return false;
	...
	public function getReturns($data = array()) {
		...
		//Start add
		$implode[] =  "EXISTS (SELECT op.order_id FROM " . DB_PREFIX . "order_product op WHERE op.vendor_id = '". (int)$this->user->getId() ."' AND op.order_id=r.order_id AND op.model=r.model)";
		//End add
